...
function run_container {
    ...
    sudo docker run --name "$name" --detach "$@"
...
function check_docker {
    if is_ubuntu; then
       ...
       rpm -q docker-ce > /dev/null 2>&1 || rpm -q docker > /dev/null 2>&1
    fi
}
...
function extract_hyperkube {
    ...
    hyperkube_container="$(sudo docker ps -aq \
        -f ancestor="${KURYR_HYPERKUBE_IMAGE}:${KURYR_HYPERKUBE_VERSION}" | \
        head -1)"
    sudo docker cp "${hyperkube_container}:/hyperkube" "$tmp_hyperkube_path"
    sudo docker cp "${hyperkube_container}:/opt/cni/bin/loopback" \
        "$tmp_loopback_cni_path"
    sudo docker cp "${hyperkube_container}:/usr/bin/nsenter" "$tmp_nsenter_path"
...
if is_service_enabled kuryr-kubernetes; then
    ...
    if [[ "$1" == "unstack" ]]; then
        ...
        sudo docker kill devstack-k8s-setup-files
        sudo docker rm devstack-k8s-setup-files
